...
describe('parser library statements', () => {
    ...
    it('does not prevent usage of `library` as varible name', () => {
        ...
        (0, chai_1.expect)(diagnostics).to.be.lengthOf(0);
        (0, chai_1.expect)(assignment).to.be.instanceOf(Statement_1.AssignmentStatement);
        (0, chai_1.expect)(assignment.name.text).to.equal('library');
    });
    it('does not prevent usage of `library` as object property name', () => {
        ...
        const { statements, diagnostics } = Parser_1.Parser.parse(tokens);
        //make sure the assignment is present in the function body
        (0, chai_1.expect)(statements[0].func.body.statements[0].value.elements[0].keyToken.text).to.equal('library');
        (0, chai_1.expect)(diagnostics).to.be.lengthOf(0);
    });
    it('parses rest of file with ONLY the library keyword present at root level', () => {
        ...
        (0, chai_1.expect)(statements[statements.length - 1]).to.be.instanceOf(Statement_1.FunctionStatement);
    });
});
