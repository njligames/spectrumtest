...
describe('parser', () => {
    describe('`end` keyword', () => {
        ...
        it('is not allowed as a standalone variable', () => {
            ...
            (0, chai_1.expect)(diagnostics[0].range).to.deep.include(vscode_languageserver_1.Range.create(1, 4, 1, 8));
        });
    });
    ...
    it('allows whitelisted reserved words as object properties', () => {
        ...
        `);
        let { diagnostics } = Parser_1.Parser.parse(tokens);
        (0, chai_1.expect)(JSON.stringify(diagnostics[0])).not.to.exist;
        (0, chai_1.expect)(diagnostics).to.be.lengthOf(0);
    });
    it('allows rem as a property name only in certain situations', () => {
        ...
        (0, chai_1.expect)(diagnostics).to.exist.and.be.lengthOf(0, 'Error count should be 0');
        (0, chai_1.expect)(statements[0].func.body.statements[0].value.elements[0].text).to.equal('rem: 1');
        (0, chai_1.expect)(statements[0].func.body.statements[1].value.elements[1].text).to.equal('rem: 2');
        (0, chai_1.expect)(statements[0].func.body.statements[2].value.elements[0].text).to.equal('rem: 3: name: "bob"');
        (0, chai_1.expect)(statements[0].func.body.statements[3].name.text).to.equal('rem');
    });
    it('handles quoted AA keys', () => {
        ...
        (0, chai_1.expect)(element.keyToken.text).to.equal('"has-second-layer"');
    });
    it('extracts property names for completion', () => {
        ...
        (0, chai_1.expect)(Object.keys(initialHints).sort()).to.deep.equal(expected, 'Initial hints');
        parser.invalidateReferences();
        const { propertyHints: refreshedHints } = parser.references;
        (0, chai_1.expect)(Object.keys(refreshedHints).sort()).to.deep.equal(expected, 'Refreshed hints');
    });
