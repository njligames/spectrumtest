...
class RokuDeploy {
    ...
    async zipPackage(options) {
        ...
        if (!await util_2.util.fileExistsCaseInsensitive(`${options.stagingFolderPath}/manifest`)) {
    ...
    async createPackage(options, beforeZipCallback) {
        ...
        let manifestPath = util_2.util.standardizePath(`${options.stagingFolderPath}/manifest`);
    ...
    async getFilePaths(files, rootDir) {
        ...
        for (let entry of normalizedFiles) {
            ...
            let entryResults = await this.getFilePathsForEntry(typeof entry === 'string' ? src : Object.assign(Object.assign({}, entry), { src: src }), rootDir);
    ...
    async getFilePathsForEntry(entry, rootDir) {
        ...
        for (let filePathAbsolute of files) {
            ...
            if (await util_2.util.isFile(filePathAbsolute)) {
    ...
    getDestPath(srcPathAbsolute, files, rootDir, skipMatch = false) {
        ...
        for (let entry of standardizedFiles) {
            ...
            if (typeof entry === 'string') {
                ...
                let srcPathRelative = util_2.util.stringReplaceInsensitive(srcPathAbsolute, rootDir, '');
            ...
            if (glob.hasMagic(entry.src) === false) {
                ...
                let entrySrcPathAbsolute = isSrcPathAbsolute ? entry.src : util_2.util.standardizePath(`${rootDir}/${entry.src}`);
                let isSrcChildOfRootDir = util_2.util.isParentOfPath(rootDir, entrySrcPathAbsolute);
                ...
                if (!entry.dest) {
                    ...
                    if (isSrcPathAbsolute || isSrcChildOfRootDir === false) {
                        ...
                        let srcPathRelative = util_2.util.stringReplaceInsensitive(entrySrcPathAbsolute, rootDir, '');
            ...
            if (entry.src.includes('**')) {
                ...
                let srcPathRelative = util_2.util.stringReplaceInsensitive(srcPathAbsolute, absolutePathToStarStar, '');
    ...
    async copyToStaging(files, stagingPath, rootDir) {
        ...
        await Promise.all(fileObjects.map(async (fileObject) => {
            let destFilePath = util_2.util.standardizePath(`${stagingPath}/${fileObject.dest}`);
            ...
            await util_2.util.tryRepeatAsync(async () => {
     ...
     * @param zipFilePath
     */
    async zipFolder(srcFolder, zipFilePath, preFileZipCallback) {
