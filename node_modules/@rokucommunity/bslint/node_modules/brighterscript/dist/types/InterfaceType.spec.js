...
describe('InterfaceType', () => {
    describe('toString', () => {
        it('returns empty curly braces when no members', () => {
            chai_1.expect(iface({}).toString()).to.eql('{}');
        });
        it('includes member types', () => {
            chai_1.expect(iface({ name: new StringType_1.StringType() }).toString()).to.eql('{ name: string; }');
        });
    ...
    describe('equals', () => {
        it('matches equal objects', () => {
            chai_1.expect(iface({ name: new StringType_1.StringType() }).equals(iface({ name: new StringType_1.StringType() }))).to.be.true;
        });
        it('does not match inequal objects', () => {
            chai_1.expect(iface({ name: new StringType_1.StringType() }).equals(iface({ name: new IntegerType_1.IntegerType() }))).to.be.false;
        });
    });
    describe('isAssignableTo', () => {
        ...
        it('matches properties in mismatched order', () => {
            chai_1.expect(new InterfaceType_1.InterfaceType(new Map([
                ['name', new StringType_1.StringType()],
                ['age', new IntegerType_1.IntegerType()]
...
function iface(members) {
    return new InterfaceType_1.InterfaceType(new Map(Object.entries(members)));
}
