"use strict";
Object.defineProperty(exports, "__esModule", { value: true });
const chai_1 = require("chai");
const Parser_1 = require("../../Parser");
const lexer_1 = require("../../../lexer");
const Parser_spec_1 = require("../Parser.spec");
const vscode_languageserver_1 = require("vscode-languageserver");
describe('parser while statements', () => {
    it('while without exit', () => {
        const { statements, diagnostics } = Parser_1.Parser.parse([
            Parser_spec_1.token(lexer_1.TokenKind.While, 'while'),
            Parser_spec_1.token(lexer_1.TokenKind.True, 'true'),
            Parser_spec_1.token(lexer_1.TokenKind.Newline, '\n'),
            Parser_spec_1.token(lexer_1.TokenKind.Print, 'print'),
            Parser_spec_1.token(lexer_1.TokenKind.StringLiteral, 'looping'),
            Parser_spec_1.token(lexer_1.TokenKind.Newline, '\n'),
            Parser_spec_1.token(lexer_1.TokenKind.EndWhile, 'end while'),
            Parser_spec_1.EOF
        ]);
        chai_1.expect(diagnostics).to.be.lengthOf(0);
        chai_1.expect(statements).to.be.length.greaterThan(0);
    });
    it('while with exit', () => {
        const { statements, diagnostics } = Parser_1.Parser.parse([
            Parser_spec_1.token(lexer_1.TokenKind.While, 'while'),
            Parser_spec_1.token(lexer_1.TokenKind.True, 'true'),
            Parser_spec_1.token(lexer_1.TokenKind.Newline, '\n'),
            Parser_spec_1.token(lexer_1.TokenKind.Print, 'print'),
            Parser_spec_1.token(lexer_1.TokenKind.StringLiteral, 'looping'),
            Parser_spec_1.token(lexer_1.TokenKind.Newline, '\n'),
            Parser_spec_1.token(lexer_1.TokenKind.ExitWhile, 'exit while'),
            Parser_spec_1.token(lexer_1.TokenKind.Newline, '\n'),
            Parser_spec_1.token(lexer_1.TokenKind.EndWhile, 'end while'),
            Parser_spec_1.EOF
        ]);
        chai_1.expect(diagnostics).to.be.lengthOf(0);
        chai_1.expect(statements).to.be.length.greaterThan(0);
    });
    it('supports trailing colon at end of condition', () => {
        var _a;
        const parser = Parser_1.Parser.parse(`
            while i > 0:
                print "while"
            end while
        `);
        chai_1.expect((_a = parser.diagnostics[0]) === null || _a === void 0 ? void 0 : _a.message).to.be.undefined;
    });
    it('catches unterminated while reaching function boundary', () => {
        const parser = Parser_1.Parser.parse(`
            function test()
                while i > 0:
                    print "while"
            end function
        `);
        chai_1.expect(parser.diagnostics).to.be.lengthOf(1);
        chai_1.expect(parser.statements).to.be.lengthOf(1);
        chai_1.expect(parser.references.functionStatements[0].func.body.statements).to.be.lengthOf(1);
    });
    it('location tracking', () => {
        var _a;
        /**
         *    0   0   0   1   1
         *    0   4   8   2   6
         *  +------------------
         * 0| while true
         * 1|   Rnd(0)
         * 2| end while
         */
        const { statements, diagnostics } = Parser_1.Parser.parse([
            {
                kind: lexer_1.TokenKind.While,
                text: 'while',
                isReserved: true,
                range: vscode_languageserver_1.Range.create(0, 0, 0, 5)
            },
            {
                kind: lexer_1.TokenKind.True,
                text: 'true',
                isReserved: true,
                range: vscode_languageserver_1.Range.create(0, 6, 0, 10)
            },
            {
                kind: lexer_1.TokenKind.Newline,
                text: '\n',
                isReserved: false,
                range: vscode_languageserver_1.Range.create(0, 10, 0, 11)
            },
            // loop body isn't significant for location tracking, so helper functions are safe
            Parser_spec_1.identifier('Rnd'),
            Parser_spec_1.token(lexer_1.TokenKind.LeftParen, '('),
            Parser_spec_1.token(lexer_1.TokenKind.IntegerLiteral),
            Parser_spec_1.token(lexer_1.TokenKind.RightParen, ')'),
            Parser_spec_1.token(lexer_1.TokenKind.Newline, '\n'),
            {
                kind: lexer_1.TokenKind.EndWhile,
                text: 'end while',
                isReserved: false,
                range: vscode_languageserver_1.Range.create(2, 0, 2, 9)
            },
            Parser_spec_1.EOF
        ]);
        chai_1.expect((_a = diagnostics[0]) === null || _a === void 0 ? void 0 : _a.message).not.to.exist;
        chai_1.expect(statements).to.be.lengthOf(1);
        chai_1.expect(statements[0].range).deep.include(vscode_languageserver_1.Range.create(0, 0, 2, 9));
    });
});
//# sourceMappingURL=While.spec.js.map