...
describe('parser call expressions', () => {
    it('parses named function calls', () => {
        const { statements, diagnostics } = Parser_1.Parser.parse([
            Parser_spec_1.identifier('RebootSystem'),
            { kind: lexer_1.TokenKind.LeftParen, text: '(', range: null },
    ...
    it('does not invalidate the next statement on a multi-statement line', () => {
        ...
        chai_1.expect(diagnostics[0].range.end.character).to.be.lessThan(25);
    });
    ...
    it('accepts arguments', () => {
        const { statements, diagnostics } = Parser_1.Parser.parse([
            ...
            { kind: lexer_1.TokenKind.Comma, text: ',', range: null },
        ...
        chai_1.expect(diagnostics).to.be.lengthOf(0);
        chai_1.expect(statements).to.be.length.greaterThan(0);
        chai_1.expect(statements[0].expression.args).to.be.ok;
    });
    it('location tracking', () => {
        ...
        chai_1.expect(statements[0].range).to.deep.include(vscode_languageserver_1.Range.create(0, 0, 0, 17));
    });
});
