...
describe('parser DimStatement', () => {
    ...
    it('flags missing identifier after dim', () => {
        ...
        chai_1.expect(dimStatement).to.exist;
        chai_1.expect(dimStatement.identifier).to.not.exist;
        chai_1.expect(parser.diagnostics.map(x => x.message)).to.include(DiagnosticMessages_1.DiagnosticMessages.expectedIdentifierAfterKeyword('dim').message);
    });
    it('flags missing left bracket', () => {
        ...
        chai_1.expect(dimStatement).to.exist;
        chai_1.expect(dimStatement.openingSquare).to.not.exist;
        chai_1.expect(parser.diagnostics.map(x => x.message)).to.include(DiagnosticMessages_1.DiagnosticMessages.missingLeftSquareBracketAfterDimIdentifier().message);
    });
    it('flags missing right bracket', () => {
        ...
        //the statement should still exist and have null dimensions
        chai_1.expect(dimStatement).to.exist;
        chai_1.expect(dimStatement.closingSquare).to.not.exist;
        chai_1.expect(parser.diagnostics.map(x => x.message)).to.include(DiagnosticMessages_1.DiagnosticMessages.missingRightSquareBracketAfterDimIdentifier().message);
    });
    it('flags missing expression(s)', () => {
        ...
        //the statement should still exist and have null dimensions
        chai_1.expect(dimStatement).to.exist;
        chai_1.expect(dimStatement.dimensions).to.be.empty;
        chai_1.expect(parser.diagnostics.map(x => x.message)).to.include(DiagnosticMessages_1.DiagnosticMessages.missingExpressionsInDimStatement().message);
    });
});
function validatePass(text, dimStatementIndex, identifierText, dimensionsCount) {
    ...
    chai_1.expect(dimStatement).to.exist;
    chai_1.expect(dimStatement.dimToken).to.exist;
    chai_1.expect(dimStatement.identifier).to.exist;
    chai_1.expect(dimStatement.identifier.text).to.equal(identifierText);
    chai_1.expect(dimStatement.openingSquare).to.exist;
    chai_1.expect(dimStatement.dimensions).to.exist;
    chai_1.expect(dimStatement.dimensions.length).to.equal(dimensionsCount);
    chai_1.expect(dimStatement.closingSquare).to.exist;
    chai_1.expect(dimStatement.range).to.exist;
}
