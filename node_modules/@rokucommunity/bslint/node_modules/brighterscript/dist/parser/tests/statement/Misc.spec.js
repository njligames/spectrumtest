...
describe('parser', () => {
    describe('`end` keyword', () => {
        ...
        it('is not allowed as a standalone variable', () => {
            ...
            chai_1.expect(diagnostics[0].range).to.deep.include(vscode_languageserver_1.Range.create(1, 4, 1, 8));
        });
    });
    ...
    it('most reserved words are not allowed as local var identifiers', () => {
        let statementList = [];
        [...lexer_1.DisallowedLocalIdentifiersText].filter(x => x === 'if').forEach((disallowedIdentifier) => {
            ...
            if (diagnostics.length === 0) {
                console.log(lexer_1.DisallowedLocalIdentifiersText);
    ...
    it('allows certain TokenKinds to be treated as local variables', () => {
        ...
        testHelpers_spec_1.expectZeroDiagnostics(Parser_1.Parser.parse(`
                sub main()
                    Void = true
                    Number = true
                    Boolean = true
    ...
    it('allows certain TokenKinds as object properties', () => {
        let kinds = [
            lexer_1.TokenKind.As,
            lexer_1.TokenKind.And,
            lexer_1.TokenKind.Box,
            lexer_1.TokenKind.CreateObject,
            lexer_1.TokenKind.Dim,
            lexer_1.TokenKind.Then,
            lexer_1.TokenKind.Else,
            lexer_1.TokenKind.End,
            lexer_1.TokenKind.EndFunction,
            lexer_1.TokenKind.EndFor,
            lexer_1.TokenKind.EndIf,
            lexer_1.TokenKind.EndSub,
            lexer_1.TokenKind.EndWhile,
            lexer_1.TokenKind.Eval,
            lexer_1.TokenKind.Exit,
            lexer_1.TokenKind.ExitFor,
            lexer_1.TokenKind.ExitWhile,
            lexer_1.TokenKind.False,
            lexer_1.TokenKind.For,
            lexer_1.TokenKind.ForEach,
            lexer_1.TokenKind.Function,
            lexer_1.TokenKind.GetGlobalAA,
            lexer_1.TokenKind.GetLastRunCompileError,
            lexer_1.TokenKind.GetLastRunRunTimeError,
            lexer_1.TokenKind.Goto,
            lexer_1.TokenKind.If,
            lexer_1.TokenKind.Invalid,
            lexer_1.TokenKind.Let,
            lexer_1.TokenKind.Next,
            lexer_1.TokenKind.Not,
            lexer_1.TokenKind.ObjFun,
            lexer_1.TokenKind.Or,
            lexer_1.TokenKind.Pos,
            lexer_1.TokenKind.Print,
            lexer_1.TokenKind.Rem,
            lexer_1.TokenKind.Return,
            lexer_1.TokenKind.Step,
            lexer_1.TokenKind.Stop,
            lexer_1.TokenKind.Sub,
            lexer_1.TokenKind.Tab,
            lexer_1.TokenKind.To,
            lexer_1.TokenKind.True,
            lexer_1.TokenKind.Type,
            lexer_1.TokenKind.While,
            lexer_1.TokenKind.Void,
            lexer_1.TokenKind.Boolean,
            lexer_1.TokenKind.Integer,
            lexer_1.TokenKind.LongInteger,
            lexer_1.TokenKind.Float,
            lexer_1.TokenKind.Double,
            lexer_1.TokenKind.String,
            lexer_1.TokenKind.Object,
            lexer_1.TokenKind.Interface,
            lexer_1.TokenKind.Dynamic,
            lexer_1.TokenKind.Void,
            lexer_1.TokenKind.As
        ].map(x => x.toLowerCase());
    ...
    it('allows whitelisted reserved words as object properties', () => {
        ...
        `);
        let { diagnostics } = Parser_1.Parser.parse(tokens);
        chai_1.expect(JSON.stringify(diagnostics[0])).not.to.exist;
        chai_1.expect(diagnostics).to.be.lengthOf(0);
    });
    it('allows rem as a property name only in certain situations', () => {
        ...
        chai_1.expect(diagnostics).to.exist.and.be.lengthOf(0, 'Error count should be 0');
        chai_1.expect(statements[0].func.body.statements[0].value.elements[0].text).to.equal('rem: 1');
        chai_1.expect(statements[0].func.body.statements[1].value.elements[1].text).to.equal('rem: 2');
        chai_1.expect(statements[0].func.body.statements[2].value.elements[0].text).to.equal('rem: 3: name: "bob"');
        chai_1.expect(statements[0].func.body.statements[3].name.text).to.equal('rem');
    });
    it('handles quoted AA keys', () => {
        ...
        chai_1.expect(element.keyToken.text).to.equal('"has-second-layer"');
    });
    it('extracts property names for completion', () => {
        ...
        chai_1.expect(Object.keys(initialHints).sort()).to.deep.equal(expected, 'Initial hints');
        parser.invalidateReferences();
        const { propertyHints: refreshedHints } = parser.references;
        chai_1.expect(Object.keys(refreshedHints).sort()).to.deep.equal(expected, 'Refreshed hints');
    });
