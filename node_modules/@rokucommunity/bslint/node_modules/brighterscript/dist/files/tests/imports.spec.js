...
let stagingFolderPath = util_1.standardizePath `${tmpPath}/staging`;
describe('import statements', () => {
    let program;
    const testTranspile = testHelpers_spec_1.getTestTranspile(() => [program, rootDir]);
    beforeEach(() => {
    ...
    it('still transpiles import statements if found at bottom of file', async () => {
        program.addOrReplaceFile('components/ChildScene.xml', testHelpers_spec_1.trim `
        ...
        chai_1.expect(testHelpers_spec_1.trimMap(fsExtra.readFileSync(`${stagingFolderPath}/components/ChildScene.xml`).toString())).to.equal(testHelpers_spec_1.trim `
            <?xml version="1.0" encoding="utf-8" ?>
            <component name="ChildScene" extends="Scene">
    ...
    it('finds function loaded in by import multiple levels deep', () => {
        ...
        let component = program.addOrReplaceFile('components/ChildScene.xml', testHelpers_spec_1.trim `
            <?xml version="1.0" encoding="utf-8" ?>
        ...
        `);
        program.validate();
        chai_1.expect(program.getDiagnostics().map(x => x.message)[0]).to.not.exist;
        chai_1.expect(component.getAvailableScriptImports().sort()).to.eql([
            util_1.standardizePath `source/intOps.bs`,
            util_1.standardizePath `source/lib.bs`,
            util_1.standardizePath `source/stringOps.bs`
        ]);
    });
    it('supports importing brs files', () => {
        ...
        program.validate();
        chai_1.expect(program.getDiagnostics().map(x => x.message)[0]).to.not.exist;
        chai_1.expect(component.getAvailableScriptImports()).to.eql([
            util_1.standardizePath `source/lib.bs`,
            util_1.standardizePath `source/stringOps.brs`
        ]);
    });
    it('detects when dependency contents have changed', () => {
        ...
        chai_1.expect(program.getDiagnostics().map(x => x.message)).to.eql([
            DiagnosticMessages_1.DiagnosticMessages.callToUnknownFunction('Waddle', util_1.standardizePath `components/ChildScene.xml`).message
        ]);
    ...
    it('adds brs imports to xml file during transpile', () => {
        //create child component
        let component = program.addOrReplaceFile({ src: util_1.standardizePath `${rootDir}/components/ChildScene.xml`, dest: 'components/ChildScene.xml' }, testHelpers_spec_1.trim `
            <?xml version="1.0" encoding="utf-8" ?>
            <component name="ChildScene" extends="ParentScene">
                <script type="text/brightscript" uri="pkg:/source/lib.bs" />
            </component>
        `);
        program.addOrReplaceFile({ src: util_1.standardizePath `${rootDir}/source/lib.bs`, dest: 'source/lib.bs' }, `
            import "stringOps.brs"
            function toLower(strVal as string)
                return StringToLower(strVal)
            end function
        `);
        program.addOrReplaceFile({ src: util_1.standardizePath `${rootDir}/source/stringOps.brs`, dest: 'source/stringOps.brs' }, `
            function StringToLower(strVal as string)
                return isInt(strVal)
            end function
        `);
        program.validate();
        chai_1.expect(testHelpers_spec_1.trimMap(component.transpile().code)).to.equal(testHelpers_spec_1.trim `
            <?xml version="1.0" encoding="utf-8" ?>
            <component name="ChildScene" extends="ParentScene">
                <script type="text/brightscript" uri="pkg:/source/lib.brs" />
                <script type="text/brightscript" uri="pkg:/source/stringOps.brs" />
    ...
    it('shows diagnostic for missing file in import', () => {
        ...
        `);
        program.validate();
        chai_1.expect(program.getDiagnostics().map(x => x.message)[0]).to.eql(DiagnosticMessages_1.DiagnosticMessages.referencedFileDoesNotExist().message);
    });
    ...
    it('handles malformed imports', () => {
        ...
        const brsFile = program.addOrReplaceFile('source/SomeFile.bs', `
            ...
            import "pkg:/"
        `);
        chai_1.expect(brsFile.ownScriptImports.length).to.equal(5);
        chai_1.expect(brsFile.ownScriptImports.filter(p => !!p.pkgPath).length).to.equal(3);
    });
});
